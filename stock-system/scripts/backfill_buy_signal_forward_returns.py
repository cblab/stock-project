#!/usr/bin/env python3
"""
Backfill missing forward returns for buy signal snapshots.

Populates forward_return_5d, forward_return_20d, and forward_return_60d
for rows in instrument_buy_signal_snapshot that have NULL values.
Uses only existing data from instrument_price_history.

Usage:
    python scripts/backfill_buy_signal_forward_returns.py [--limit N] [--dry-run] [--verbose]

Examples:
    # Dry run to see what would be updated
    python scripts/backfill_buy_signal_forward_returns.py --dry-run

    # Process all missing forward returns
    python scripts/backfill_buy_signal_forward_returns.py

    # Process only 100 snapshots (useful for testing)
    python scripts/backfill_buy_signal_forward_returns.py --limit 100
"""
...
from db.buy_signal_snapshot import BuySignalForwardReturnBackfill
...
def backfill_snapshots(
    ...
    backfill = BuySignalForwardReturnBackfill(conn)
...
def main():
    parser = argparse.ArgumentParser(
        description="Backfill missing forward returns for buy signal snapshots"
    )
    parser.add_argument(
        "--limit",
        type=int,
