...
def backfill_snapshots(
    ...
    for idx, snap in enumerate(snapshots, 1):
        ...
        try:
            ...
            has_new_data = (
                (fr_5d is not None and snap["forward_return_5d"] is None) or
                (fr_20d is not None and snap["forward_return_20d"] is None) or
                (fr_60d is not None and snap["forward_return_60d"] is None)
            )
            ...
            if dry_run:
                ...
                updated = backfill.update_forward_returns(
                    instrument_id,
                    as_of_date,
                    forward_return_5d=fr_5d,
                    forward_return_20d=fr_20d,
                    forward_return_60d=fr_60d,
                )
                if updated:
